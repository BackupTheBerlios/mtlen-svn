...
{
	context->dataLength[0]=context->dataLength[1]=0;
	context->bufferPos=0;
...
void md5_transform(MD5* context, unsigned char *input)
{
	int i;
...
//	printf(" 2.append%d\n",inputLen);
	...
	context->bufferPos+=inputLen<<3;
	context->bufferPos&=511;
//	printf("bp:%d\n",bufferPos);
}
...
//	printf("pad %d    %d.%d\n",dataLength[0],dataLength[0]>>3,dataLength[0]&7);
//	printf("pad len %d\n",padLen);
	bufferIndex=context->bufferPos>>3;
	padLen=bufferIndex<56?56-bufferIndex:64+56-bufferIndex;
    // dword to byte
	dataL[0]=context->dataLength[0];
	dataL[1]=context->dataLength[1];
	for (i=0;i<padLen;i++) {
		if (i==0) padding[i]=0x80;
		else padding[i]=0x00;
	}
	md5_update(context,padding,padLen);
 	md5_update(context,(unsigned char *)dataL, 8);
