...
int startRVPClient(int desiredStatus) {
   DBVARIANT dbv;
   char jidStr[128];
   //char server[256];
   char username[256];
...
*/
   ...
   // User may change status to OFFLINE while we are connecting above
      ...
      if (DBGetContactSettingByte(NULL, rvpProtoName, "ManualServer", FALSE) == TRUE) {
         if (!DBGetContactSetting(NULL, rvpProtoName, "LoginServer", &dbv)) {
            result = rvpimpl.signIn(username, dbv.pszVal, desiredStatus);
            DBFreeVariant(&dbv);
         } else {
            result = rvpimpl.signIn(username, NULL, desiredStatus);
         }
      } else {
         result = rvpimpl.signIn(username, NULL, desiredStatus);
      }
...
{   int result;
   ...
   int desiredStatus = (int) vPtr;
   result = startRVPClient(desiredStatus);
   if (result != 0) {
      char str [1024];
   ...
   desiredStatus = wParam;
    ...
    if (desiredStatus == ID_STATUS_OFFLINE) {
      ...
      if (!jabberConnected) {
         ...
         ProtoBroadcastAck(rvpProtoName, NULL, ACKTYPE_STATUS, ACKRESULT_SUCCESS, (HANDLE) oldStatus, jabberStatus);
         Utils::forkThread((void (__cdecl *)(void*))SignInThread, 0, (void *) desiredStatus);
      } else {
         // change status
         oldStatus = jabberStatus;
         jabberStatus = desiredStatus;
