...
void MD5::update(unsigned char *input, unsigned  int inputLen) {
	...
	dataLength[0]+=inputLen<<3;
	bufferIndex=bufferPos>>3;
	appendLen=64-bufferIndex;
	if (appendLen<=inputLen) {
		for (i=0;i<appendLen;i++) {
			buffer[bufferIndex+i]=*input;
			input++;
 		}
	 	transform(buffer);
...
void MD5::finalize() {
    ...
    // dword to byte
	dataL[0]=dataLength[0];
	dataL[1]=dataLength[1];
	for (i=0;i<padLen;i++) {
		if (i==0) padding[i]=0x80;
		else padding[i]=0x00;
	}
	update(padding,padLen);
 	update((unsigned char *)dataL, 8);
}

void MD5::get(unsigned int output[4]) {
	output[0] = state[0];
	output[1] = state[1];
	output[2] = state[2];
	output[3] = state[3];
...
void MD5::getHex(char *output) {
	int i;
	for (i=0;i<16;i++) {
		int lo, hi;
		unsigned int j=state[i>>2];//>>8*(i%4);
		j>>=8*(i%4);
		j&=0xFF;
		lo=j&0x0F;
		hi=j>>4;
